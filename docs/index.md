...
In this example, when the user selects a precision from the dropdown, the `step` property inside the number input is automatically updated, without making a server request and the server update is deferred until you explicitly call `$wire.commit()` or a different method.
