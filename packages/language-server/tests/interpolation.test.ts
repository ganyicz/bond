...
test('interpolates expressions', () => {
    ...
    expect(interpolateExpression('amount.value++', '__CTX', ts).content).toBe(
        '__CTX.amount.value++',
    );
    expect(interpolateExpression('value + {{ $this->value }}', '__CTX', ts).content).toBe(
        '__CTX.value + __BLADE__',
    );
    expect(interpolateExpression('{{ $this->value }} + value', '__CTX', ts).content).toBe(
        '__BLADE__ + __CTX.value',
    );
    expect(interpolateExpression("'{{ $this->value }}' + value", '__CTX', ts).content).toBe(
        "'__BLADE__' + __CTX.value",
    );
    expect(
        interpolateExpression('{{ $this->value }} + value + {{ $value }}', '__CTX', ts).content,
    ).toBe('__BLADE__ + __CTX.value + __BLADE__');
    expect(interpolateExpression('value + @js($this->value)', '__CTX', ts).content).toBe(
        '__CTX.value + __BLADE__',
    );
    expect(interpolateExpression('() => { value++ }', '__CTX', ts).content).toBe(
        '() => { __CTX.value++ }',
    );
    expect(interpolateExpression('calculate(amount, step)', '__CTX', ts).content).toBe(
        '__CTX.calculate(__CTX.amount, __CTX.step)',
    );
    expect(interpolateExpression('Math.max(value, 10)', '__CTX', ts).content).toBe(
        'Math.max(__CTX.value, 10)',
    );
    expect(interpolateExpression('true && value', '__CTX', ts).content).toBe('true && __CTX.value');
    expect(interpolateExpression('(x) => x + value', '__CTX', ts).content).toBe(
        '(x) => x + __CTX.value',
    );
    expect(interpolateExpression('({ red: value > 500 })', '__CTX', ts).content).toBe(
        '({ red: __CTX.value > 500 })',
    );
    expect(interpolateExpression('{ red: value > 500 }', '__CTX', ts).content).toBe(
        '{ red: __CTX.value > 500 }',
    );
    expect(interpolateExpression('for (const item in items) {}', '__CTX', ts).content).toBe(
        'for (const item in __CTX.items) {}',
    );
