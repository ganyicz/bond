...
import { extractAttributes, ExtractedSourceCode, extractMountFunction, extractPreMount } from '@bond/language-core';
...
`type Merge<T extends object[], Acc extends object = {}> =
  T extends [infer First extends object, ...infer Rest extends object[]]
    ...
    generated += 'declare function mount<TProps extends object, TReturn extends object>(callback: (props: TProps) => ThisType<TReturn & Magics<TReturn>> & TReturn): TReturn & { props: TProps } & Magics<TReturn>\n\n';
...
export function generateContext(name: string, extracted: ExtractedSourceCode, indented: boolean = false, merge: string|undefined = undefined): GeneratedCode
{
...
export function* generate(content: string, ts: typeof import('typescript')): Generator<GeneratedCode> {
    yield {
    ...
    const resetScope = function* (depth: number, range: [number, number]): Generator<GeneratedCode> {
        while(currentScope && (
    ...
    for (let i = 0; i < attributes.length; i++) {
        ...
        if (attribute.name == 'x-data') {
            ...
            const interpolated = interpolateForStatement(attribute.code.content, currentContext.name, ts, scopeVars)
            ...
            const prefix = '\t'
            const interpolated = interpolateExpression(attribute.code.content, currentContext.name, ts, scopeVars)
