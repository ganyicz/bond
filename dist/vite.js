...
function hasScriptSetup(code) {
  return /<script\s+setup\b[^>]*>/.test(code);
}
function extractScriptSetupContent(code) {
...
function extractMountFunction(code) {
  const scriptSetup = extractScriptSetupContent(code);
  if (!scriptSetup) return void 0;
...
`);
        ...
        const expressions = extractAttributes(code).map((attr) => JSON.stringify({
          name: attr.name,
...
${expressions.join(",\n")}
        ...
        return {
          code: ms.toString(),
          map: isDevMode ? ms.generateMap({ source: fileName, includeContent: false }) : null
        };
      }
    }
  };
}
function findBladeFilePaths(dir) {
